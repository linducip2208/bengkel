...
class SitemapBuilder
{
    ...
    {
        return [
            'pages' => fn () => count($this->pageUrls(rtrim(config('app.url'), '/'))),
            'blogs' => fn () => count($this->blogUrls(rtrim(config('app.url'), '/'))),
            'pseo-basic' => 50,
    ...
    private function pageUrls(string $base): array
    {
        ...
        if (class_exists(DocsController::class)) {
            foreach ((new DocsController)->sections() as $section) {
                $urls[] = ['loc' => "$base/docs/{$section['slug']}", 'priority' => '0.7'];
            }
        }
    ...
    private function blogUrls(string $base): array
    {
        ...
        if (class_exists(BlogCategory::class)) {
            $cats = BlogCategory::query()->get(['slug']);
            foreach ($cats as $cat) {
                if ($cat->slug) {
                    $urls[] = ['loc' => "$base/blog/category/{$cat->slug}", 'priority' => '0.6'];
                }
            }
        }
