...
class PaymentGatewayController extends Controller
{
    ...
    {
        return $this->save($request, $paymentGateway);
    }

    public function destroy(PaymentGateway $paymentGateway)
    {
        if ($paymentGateway->is_default) {
            return back()->with('error', 'PG default tidak bisa dihapus, set yang lain dulu sebagai default.');
        }
    ...
    {
        try {
            $link = $service->handleCallback($token, $request->all(), $request->headers->all(), $request->getContent());
        ...
        DB::transaction(function () use ($validated, $extraHeaders, $extraConfig, $methods, $request, &$gateway) {
            ...
            if ($gateway) {
                ...
                if (! empty($validated['secret_key'])) {
                    $gateway->secret_key = $validated['secret_key'];
                }
                $gateway->save();
            } else {
