...
class RoleController extends Controller
{
    ...
    public function roleStore(Request $request)
    {
        ...
        if (!empty($validated['permissions'])) $role->syncPermissions($validated['permissions']);
    ...
    {
        if (in_array($role->name, ['super_admin', 'admin'])) return back()->with('error', 'Role sistem tidak bisa dihapus.');
        if ($role->users()->count() > 0) return back()->with('error', "Role {$role->name} masih dipakai user, tidak bisa dihapus.");
