...
class VoucherController extends Controller
{
    ...
    public function destroy(Voucher $voucher)
    {
        if ($voucher->used_count > 0) {
            return back()->with('error', 'Voucher tidak bisa dihapus karena sudah digunakan.');
        }
        $voucher->delete();
    ...
    public function validateCode(Request $request): JsonResponse
    {
        ...
        if (! $voucher) {
            return response()->json(['ok' => false, 'error' => 'Kode tidak ditemukan']);
        }
        if (! $voucher->isUsable()) {
            return response()->json(['ok' => false, 'error' => 'Voucher tidak aktif/sudah habis/expired']);
        }
        if ($subtotal < $voucher->min_purchase) {
            return response()->json(['ok' => false, 'error' => 'Minimum pembelian Rp '.number_format($voucher->min_purchase, 0, ',', '.')]);
        }
