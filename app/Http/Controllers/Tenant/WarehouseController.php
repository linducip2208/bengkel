...
class WarehouseController extends Controller
{
    ...
    public function transferStore(Request $request)
    {
        ...
        // Sort by product_id for a deterministic row-lock order.
        usort($v['items'], fn ($a, $b) => $a['product_id'] <=> $b['product_id']);

        DB::transaction(function () use ($v, &$transfer) {
            $transfer = StockTransfer::create([
                'transfer_number' => DocumentNumberService::generate(DocumentNumberService::STOCK_TRANSFERS, 'TRF', 'Ym', 4),
            ...
            foreach ($v['items'] as $item) {
                ...
                // Kurangi stok dari gudang asal — locked & sufficiency-checked.
                $fromStock = WarehouseStock::where('warehouse_id', $v['from_warehouse_id'])
                    ->where('product_id', $item['product_id'])
                    ->lockForUpdate()
                    ->first();
                if (! $fromStock) {
                    $fromStock = WarehouseStock::create(['warehouse_id' => $v['from_warehouse_id'], 'product_id' => $item['product_id'], 'quantity' => 0]);
                }

                if ((float) $fromStock->quantity < (float) $item['quantity']) {
                    $name = Product::find($item['product_id'])?->name ?? "ID {$item['product_id']}";
                    throw new \RuntimeException("Stok \"{$name}\" di gudang asal tidak cukup: tersedia {$fromStock->quantity}, dibutuhkan {$item['quantity']}.");
                }

                $fromPrevious = (float) $fromStock->quantity;
                $fromStock->decrement('quantity', $item['quantity']);
                StockHistory::create([
                    'product_id' => $item['product_id'],
                    'quantity_change' => -$item['quantity'],
                    'previous_stock' => $fromPrevious,
                    'new_stock' => round($fromPrevious - (float) $item['quantity'], 2),
                    'type' => 'transfer_out',
                ...
                $toStock = WarehouseStock::where('warehouse_id', $v['to_warehouse_id'])
                    ->where('product_id', $item['product_id'])
                    ->lockForUpdate()
                    ->first();
                if (! $toStock) {
                    try {
                        $toStock = WarehouseStock::create(['warehouse_id' => $v['to_warehouse_id'], 'product_id' => $item['product_id'], 'quantity' => 0]);
                    } catch (UniqueConstraintViolationException) {
                        $toStock = WarehouseStock::where('warehouse_id', $v['to_warehouse_id'])->where('product_id', $item['product_id'])->lockForUpdate()->first();
                    }
                }
                $toPrevious = (float) $toStock->quantity;
                $toStock->increment('quantity', $item['quantity']);
                StockHistory::create([
                    'product_id' => $item['product_id'],
                    'quantity_change' => $item['quantity'],
                    'previous_stock' => $toPrevious,
                    'new_stock' => round($toPrevious + (float) $item['quantity'], 2),
                    'type' => 'transfer_in',
                    'reference_type' => StockTransfer::class,
                    'reference_id' => $transfer->id,
                    'reason' => 'Transfer #'.$transfer->transfer_number,
