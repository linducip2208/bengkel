@extends('layouts.app')
@section('title', 'Buat Kontrak Fleet')
@section('content')
<div class="d-flex justify-content-between mb-4">
    <h4><i class="fas fa-file-signature me-2"></i>Buat Kontrak Fleet Baru</h4>
    <a href="{{ route('fleet-contracts.index') }}" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i>Kembali</a>
</div>
<div class="card"><div class="card-body">
<form action="{{ route('fleet-contracts.store') }}" method="POST">
    @csrf
    <div class="row g-3">
        <div class="col-md-6">
            <label class="form-label">Nama Kontrak <span class="text-danger">*</span></label>
            <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name') }}" placeholder="Contoh: Kontrak Fleet PT. ABC 2026" required>
        ...
        <div class="col-md-3">
            ...
            <input type="number" min="1" name="service_interval_days" class="form-control @error('service_interval_days') is-invalid @enderror" value="{{ old('service_interval_days', 90) }}">
        ...
        <div class="col-md-3">
            ...
            <input type="number" min="0" name="service_interval_km" class="form-control @error('service_interval_km') is-invalid @enderror" value="{{ old('service_interval_km', 5000) }}">
        ...
        <div class="col-12">
            ...
            <select name="vehicle_ids[]" multiple class="form-select @error('vehicle_ids') is-invalid @enderror" size="8">
                ...
                <option value="{{ $v->id }}" {{ in_array($v->id, old('vehicle_ids', [])) ? 'selected' : '' }}>
    ...
    <div class="mt-4">
        <button type="submit" class="btn btn-primary px-4"><i class="fas fa-save me-1"></i>Simpan Kontrak</button>
        <a href="{{ route('fleet-contracts.index') }}" class="btn btn-outline-secondary ms-2">Batal</a>
    </div>
</form>
</div></div>
@endsection
