@extends('layouts.app')
@section('title', 'Edit Observation Point')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="fas fa-clipboard-check me-2"></i>Edit Observation Point</h4>
    <a href="{{ route('observation-points.index') }}" class="btn btn-outline-secondary"><i class="fas fa-arrow-left me-1"></i>Kembali</a>
</div>
<div class="card"><div class="card-body">
    <form action="{{ route('observation-points.update', $observationPoint) }}" method="POST">
        @csrf @method('PUT')
        <div class="row g-3">
            <div class="col-md-5">
                ...
                <select name="observation_type_id" class="form-select @error('observation_type_id') is-invalid @enderror" required>
                    @foreach($observationTypes as $ot)
                        <option value="{{ $ot->id }}" {{ old('observation_type_id', $observationPoint->observation_type_id) == $ot->id ? 'selected' : '' }}>{{ $ot->observation_type }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-7">
                ...
                <input type="text" name="observation_point" class="form-control @error('observation_point') is-invalid @enderror" value="{{ old('observation_point', $observationPoint->observation_point) }}" required>
