...
<form action="{{ route('purchase-orders.update', $purchaseOrder) }}" method="POST" id="purchase-order-form">
    ...
    <div class="row g-3 mb-3">
        <div class="col-md-3">
            ...
            <input type="text" class="form-control-plaintext" value="{{ $purchaseOrder->po_number }}" readonly>
        </div>
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            ...
            <input type="date" name="order_date" id="order_date" class="form-control form-control-sm @error('order_date') is-invalid @enderror" value="{{ old('order_date', $purchaseOrder->order_date->format('Y-m-d')) }}" required>
            @error('order_date') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        <div class="col-md-3">
            <label for="expected_date" class="form-label">Estimasi Tiba</label>
            <input type="date" name="expected_date" id="expected_date" class="form-control form-control-sm @error('expected_date') is-invalid @enderror" value="{{ old('expected_date', $purchaseOrder->expected_date?->format('Y-m-d')) }}">
    ...
    <div class="row g-3 mb-3">
        ...
        <div class="col-md-4">
            ...
            <select name="status" id="status" class="form-select form-select-sm @error('status') is-invalid @enderror">
                ...
                <option value="sent" {{ old('status', $purchaseOrder->status) == 'sent' ? 'selected' : '' }}>Terkirim</option>
            </select>
            @error('status') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        <div class="col-md-4">
            <label for="tax_amount" class="form-label">Pajak (Rp)</label>
            <input type="number" name="tax_amount" id="tax_amount" step="0.01" class="form-control form-control-sm @error('tax_amount') is-invalid @enderror" value="{{ old('tax_amount', $purchaseOrder->tax_amount) }}" min="0">
    ...
    <div class="row g-3">
        <div class="col-md-12">
            ...
            <textarea name="notes" id="notes" rows="3" class="form-control form-control-sm @error('notes') is-invalid @enderror">{{ old('notes', $purchaseOrder->notes) }}</textarea>
...
@push('scripts')
<script>
    const existingItems = @json($purchaseOrder->items->map(function ($i) {
        return ['product_id' => $i->product_id, 'description' => $i->description, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price];
    }));
