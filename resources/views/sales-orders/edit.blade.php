...
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0">Edit Sales Order</h4>
    <a href="{{ route('sales-orders.show', $salesOrder) }}" class="btn btn-outline-secondary btn-sm">
...
<form action="{{ route('sales-orders.update', $salesOrder) }}" method="POST" id="sales-order-form">
    ...
    <div class="row g-3 mb-3">
        <div class="col-md-4">
            ...
            <input type="text" class="form-control-plaintext" value="{{ $salesOrder->order_number }}" readonly>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <input type="date" name="order_date" id="order_date" class="form-control form-control-sm @error('order_date') is-invalid @enderror" value="{{ old('order_date', $salesOrder->order_date->format('Y-m-d')) }}" required>
    ...
    <div class="row g-3 mb-3">
        <div class="col-md-4">
            ...
            <select name="vehicle_id" id="vehicle_id" class="form-select form-select-sm @error('vehicle_id') is-invalid @enderror">
                ...
                @foreach($vehicles as $vehicle)
                    <option value="{{ $vehicle->id }}" {{ old('vehicle_id', $salesOrder->vehicle_id) == $vehicle->id ? 'selected' : '' }}>
        ...
        <div class="col-md-4">
            ...
            <select name="status" id="status" class="form-select form-select-sm @error('status') is-invalid @enderror">
                ...
                <option value="sent" {{ old('status', $salesOrder->status) == 'sent' ? 'selected' : '' }}>Terkirim</option>
                <option value="approved" {{ old('status', $salesOrder->status) == 'approved' ? 'selected' : '' }}>Disetujui</option>
            </select>
    ...
    <div class="row g-3 mb-3">
        <div class="col-md-4">
            ...
            <input type="number" name="discount" id="discount" step="0.01" class="form-control form-control-sm @error('discount') is-invalid @enderror" value="{{ old('discount', $salesOrder->discount) }}" min="0">
            @error('discount') <div class="invalid-feedback">{{ $message }}</div> @enderror
        </div>
        <div class="col-md-4">
            <label for="tax_amount" class="form-label">Pajak (Rp)</label>
            <input type="number" name="tax_amount" id="tax_amount" step="0.01" class="form-control form-control-sm @error('tax_amount') is-invalid @enderror" value="{{ old('tax_amount', $salesOrder->tax_amount) }}" min="0">
    ...
    <div class="row g-3">
        <div class="col-md-12">
            ...
            <textarea name="notes" id="notes" rows="3" class="form-control form-control-sm @error('notes') is-invalid @enderror">{{ old('notes', $salesOrder->notes) }}</textarea>
    ...
    <div class="mt-3">
        ...
        <a href="{{ route('sales-orders.show', $salesOrder) }}" class="btn btn-outline-secondary">Batal</a>
    </div>
</form>
@endsection
...
@push('scripts')
<script>
    const existingItems = @json($salesOrder->items->map(function ($i) {
        return ['product_id' => $i->product_id, 'description' => $i->description, 'quantity' => $i->quantity, 'unit_price' => $i->unit_price, 'discount' => $i->discount];
    }));
