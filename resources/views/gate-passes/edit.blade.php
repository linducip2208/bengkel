...
@section('title', 'Edit Gate Pass - ' . $gatePass->gate_pass_no)

@section('content')
<h4 class="mb-3">Edit Gate Pass: {{ $gatePass->gate_pass_no }}</h4>
                ...
                <div class="col-md-6">
                    ...
                    <select name="service_id" class="form-select @error('service_id') is-invalid @enderror">
                        ...
                        @foreach($services as $service)
                            <option value="{{ $service->id }}" {{ old('service_id', $gatePass->service_id) == $service->id ? 'selected' : '' }}>
            ...
            <div class="row mb-3">
                <div class="col-md-6">
                    ...
                    <input type="datetime-local" name="entry_date" class="form-control @error('entry_date') is-invalid @enderror"
                           value="{{ old('entry_date', $gatePass->entry_date->format('Y-m-d\TH:i')) }}" required>
                    @error('entry_date')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6">
                    <label class="form-label">Driver Name</label>
                    <input type="text" name="driver_name" class="form-control" value="{{ old('driver_name', $gatePass->driver_name) }}">
                </div>
            </div>
            <div class="row mb-3">
                <div class="col-md-6">
                    <label class="form-label">Driver Phone</label>
                    <input type="text" name="driver_phone" class="form-control" value="{{ old('driver_phone', $gatePass->driver_phone) }}">
                </div>
            </div>
            <div class="mb-3">
                <label class="form-label">Notes</label>
                <textarea name="notes" class="form-control" rows="2">{{ old('notes', $gatePass->notes) }}</textarea>
            </div>
            <button type="submit" class="btn btn-primary"><i class="bi bi-save"></i> Update Gate Pass</button>
