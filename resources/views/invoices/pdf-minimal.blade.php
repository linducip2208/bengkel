...
<head>
    ...
    <style>
        ...
        body { font-family: 'Inter', Helvetica, Arial, sans-serif; font-size: 12px; color: #1a1a1a; margin: 0; padding: 35px 40px; line-height: 1.6; }
        ...
        .top-bar .invoice-label .tag { display: inline-block; padding: 4px 14px; border-radius: 20px; font-size: 10px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; }
...
<div class="top-bar">
    <div style="display:flex;align-items:center;gap:14px;">
        @if(!empty($settings['logo']) && file_exists(public_path('storage/' . $settings['logo'])))
            <img src="{{ public_path('storage/' . $settings['logo']) }}" style="width:40px;height:40px;border-radius:6px;">
        @endif
        <div>
...
<table class="items">
    <thead>
        <tr>
            ...
            <th width="22%" class="text-end">Jumlah</th>
        </tr>
    </thead>
    <tbody>
        @foreach ($invoice->items as $idx => $item)
            <tr>
                <td class="text-center" style="color:#bbb;">{{ $idx + 1 }}</td>
                <td>{{ $item->description }}</td>
                <td class="text-center">{{ $item->quantity }}</td>
                <td class="text-end">@money($item->unit_price)</td>
                <td class="text-end" style="font-weight:500;">@money($item->total_price)</td>
            </tr>
        @endforeach
    </tbody>
</table>
    ...
    <table>
        @foreach ($invoice->paymentRecords as $p)
            <tr>
                ...
                <td class="text-end" style="font-weight:500;">@money($p->amount)</td>
                <td class="text-end" style="color:#aaa;font-size:10px;">{{ $p->reference_number }}</td>
            </tr>
        @endforeach
    </table>
</div>
