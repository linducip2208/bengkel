@extends('layouts.app')
@section('title', 'Edit Merek Kendaraan')
@section('content')
<div class="d-flex justify-content-between align-items-center mb-4">
    <h4 class="mb-0"><i class="fas fa-trademark me-2"></i>Edit Merek Kendaraan</h4>
    ...
    <form action="{{ route('vehicle-brands.update', $vehicleBrand) }}" method="POST">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                ...
                <select name="vehicle_type_id" class="form-select @error('vehicle_type_id') is-invalid @enderror" required>
                    ...
                    @foreach($vehicleTypes as $vt)
                        <option value="{{ $vt->id }}" {{ old('vehicle_type_id', $vehicleBrand->vehicle_type_id) == $vt->id ? 'selected' : '' }}>{{ $vt->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="col-md-6">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" value="{{ old('name', $vehicleBrand->name) }}" required>
