...
<head>
    <meta charset="UTF-8">
    <title>Setujui Estimasi — {{ config('app.name') }}</title>
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="robots" content="noindex">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.css" rel="stylesheet">
    <style>
        body { background: linear-gradient(135deg, #166534 0%, #22c55e 100%); min-height: 100vh; padding: 1.5rem; }
        ...
        <div class="card">
            <div class="card-body p-4">
                <div class="text-center mb-4">
                    ...
                    <h4 class="mt-2 mb-0">Setujui estimasi servis ini?</h4>
                ...
                <div class="table-responsive mb-4">
                    <table class="table table-borderless table-sm mb-0">
                        ...
                        <tr><td class="text-muted">Detail</td><td>{{ $service->description }}</td></tr>
                        @endif
                ...
                <div class="d-grid gap-2">
                    <form method="POST" action="{{ route('public.approval.approve.store', $service->approval_token) }}">
                        @csrf
                        <button class="btn btn-success btn-lg w-100"><i class="bi bi-check-circle me-1"></i>Setujui Estimasi</button>
                    </form>
                    <a href="{{ route('public.approval.reject', $service->approval_token) }}" class="btn btn-outline-danger">Tolak Estimasi</a>
                </div>
                @else
                <div class="alert alert-success text-center mb-0">
                    <i class="bi bi-check-circle-fill me-1"></i>Estimasi ini sudah disetujui pada {{ $service->approved_at?->format('d M Y H:i') }}.
                </div>
                @endif
            </div>
        </div>
