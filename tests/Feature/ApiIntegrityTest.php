...
class ApiIntegrityTest extends TestCase
{
    ...
    public function test_api_jobcard_cannot_inject_done_status(): void
    {
        $this->authed('service_advisor');
        $service = $this->makeService();

        // Attempting to force completion via a raw status write must be ignored
        // (done_status is system-owned; completion routes through executeComplete).
        $this->putJson("/api/v1/jobcards/{$service->id}", [
            'title' => 'Updated title',
            'done_status' => 2,
        ])->assertOk();

        $fresh = $service->fresh();
        $this->assertEquals(0, (int) $fresh->done_status, 'done_status must not be client-injectable.');
        $this->assertEquals('Updated title', $fresh->title);
        $this->assertNull(Invoice::where('service_id', $service->id)->first(), 'No invoice may be generated by a raw status write.');
    }

    public function test_api_jobcard_complete_runs_through_service_service(): void
    {
        $this->authed('service_advisor');
        $service = $this->makeService();

        $response = $this->postJson("/api/v1/jobcards/{$service->id}/complete");
        $response->assertOk();

        $this->assertSame(2, (int) $service->fresh()->done_status);
        $this->assertSame(1, Invoice::where('service_id', $service->id)->count());

        // Idempotent — a second completion does not create another invoice.
        $this->postJson("/api/v1/jobcards/{$service->id}/complete")->assertOk();
        $this->assertSame(1, Invoice::where('service_id', $service->id)->count());
    }
}
