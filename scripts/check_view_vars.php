...
foreach ($ctrlDirs as $d) {
    ...
    foreach ($rii as $fi) {
        ...
        if (! preg_match_all('/public function (\w+)\([^)]*\)[^{]*\{((?:[^{}]|\{[^{}]*\})*)\}/s', $src, $methods, PREG_SET_ORDER)) {
            continue;
        }
        ...
        foreach ($methods as $mm) {
            ...
            if (! preg_match_all("/view\(['\"]([\w.\-_]+)['\"]\s*,\s*(compact\([^)]+\)|\[[^\]]*\])?\)?/", $body, $vm, PREG_SET_ORDER)) {
                continue;
            }
...
foreach ($rii as $fi) {
    if ($fi->isDir() || ! str_ends_with($fi->getFilename(), '.blade.php')) {
        continue;
    }
    ...
    $declared = [];
    if (preg_match_all('/@php\s*\$(\w+)\s*=/', $src, $pm)) {
        $declared = array_merge($declared, $pm[1]);
    }
    if (preg_match_all('/@foreach\s*\(\s*\$\w+(?:->\w+|\[[^\]]+\])*\s+as\s+(?:\$(\w+)\s*=>\s*)?\$(\w+)/', $src, $fm)) {
        $declared = array_merge($declared, $fm[1], $fm[2]);
    }
    if (preg_match_all('/@for\s*\(\s*\$(\w+)/', $src, $fmm)) {
        $declared = array_merge($declared, $fmm[1]);
    }
    if (preg_match_all('/@forelse\s*\(\s*\$\w+\s+as\s+\$(\w+)/', $src, $fem)) {
        $declared = array_merge($declared, $fem[1]);
    }
    if (preg_match_all('/@\s*foreach\s*\([^)]+as\s+\$(\w+)/', $src, $fem2)) {
        $declared = array_merge($declared, $fem2[1]);
    }

    $allRefs = [];
    if (preg_match_all('/\$(\w+)/', $src, $rm)) {
        $allRefs = $rm[1];
    }
